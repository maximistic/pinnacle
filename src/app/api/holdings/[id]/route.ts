...
export async function PUT(request: NextRequest, { params }: Params) {
  try {
    ...
    const { type, name, quantity, investedValue, currentValue, notes, source, isin, folioNumber, currency, exchangeRate, ticker, amfi } =
      body;
    ...
    const holding = await prisma.holding.update({
      ...
      data: {
        ...
        ...(ticker !== undefined && {
          ticker: typeof ticker === "string" ? ticker.trim() || null : null,
        }),
        ...(amfi !== undefined && {
          amfi: typeof amfi === "string" ? amfi.trim() || null : null,
        }),
      },
    });
